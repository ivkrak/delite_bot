...
#     """
#     sessions_data = []
#     for session_name in [f.replace('.session', '') for f in os.listdir('sessions') if f.endswith('.session')]:
...
def generate_time_for_sessions():
    ...
    for session_name in [f for f in os.listdir('sessions') if f.endswith('.session')]:
        ...
        with open(f'sessions/{session_name}_time.json', 'w') as f:
            time_info = {"session_name": session_name, "used_at": time.time()}
            print(time_info)
            time.sleep(1)
